    ...
    def store_in_database(self):
        ...
        if check_value == 0:
            try:
                ...
                cursor.execute(
                    'CREATE TABLE IF NOT EXISTS STUDENT (name TEXT NOT NULL, roll TEXT UNIQUE, year TEXT NOT NULL)')
                cursor.execute('INSERT INTO STUDENT (name, roll, year) VALUES(?, ?, ?)',
                               (str(self.e1.text()), str(self.e2.text()), str(self.year)))
                connection.commit()
                cursor.close()
                connection.close()
